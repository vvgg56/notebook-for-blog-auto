...
- [🆕🔴 발행체계 전환: jgluna용GUI v2.51→v2.52 + 서버 bizops 레이어](reference_jgluna_gui_v251_bizops.md) — 2026-08-10 TabPublisher→biz워커 교체. 서버 bizops_compat(워커API)+title_biz_ai+딸칵+브랜딩7 분리(타세션)+/remote 브리지, customers 57(엘리트=7월 IP 복원). v2.52=/remote 원버튼화(워커 자동시작, 소스 ~/Desktop/biz-publisher), IP중복=state overlay 함정(expected-ip API로 base값 복원, ""금지)
