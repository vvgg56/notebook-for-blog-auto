...
**2026-07-13 라벨 추가**: 프로필 목록(최초1회 로그인·연속발행 탭)이 엘리트 모드일 때 배정 엘리트IP 표시(공용 `_ip_label(blog,bracket)` = `_elite_ip_for(vpn_id)`, "엘리트IP 211.x.x.x"/"엘리트IP 없음", esvpn 모드 불변). blog-publisher main push됨. 재빌드→`C:\Users\장영훈\Pictures\blog용GUI_v1.2.1\blog용GUI_v1.2.1.exe.new` 대기(GUI 실행중이라 exe 잠김 — 종료 후 .new로 교체). ⚠️엘리트 라벨은 vpn_id로 조회(첫10블로그 blog_id==vpn_id라 무관, override블로그는 blog_ips 키를 vpn_id로 넣어야).
